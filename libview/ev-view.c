                      ...
                      GdkEventScroll *event)
{
     ...
     * CTRL + WHEEL
     *
     * Tetap digunakan untuk zoom PDF.
     * ============================================================
     */
...
/*
    ...
    if (paragraph_text) {
        ...
        g_print ("\n");
        g_print ("========================================\n");
        g_print ("TRANSLATE BUTTON CLICKED\n");
        g_print ("PAGE  : %d\n",
                 view->translate_page);
        g_print ("INDEX : %d\n",
                 view->translate_index);
                       ...
                       gint y)
{
    ...
    if (!view->translate_window) {
        ...
        /*
         * Callback button Translate.
         *
         * view menjadi user_data.
         */
        g_signal_connect (
            button,
            "clicked",
...
static gchar *ev_process_translate(EvView *view,
    ...
    gchar *path_translate = g_strdup_printf("%s_translated/original/%d", filename, page);
    gchar *result_translate = g_strdup_printf("%s_translated/translate/%d/%d", filename, page, paragraf);
    gchar *file_translate = g_strdup_printf("%s/%d", path_translate, paragraf);

    // Buat folder kalau belum ada
    struct stat st = {0};
    if (stat(path_translate, &st) == -1) {
        if (g_mkdir_with_parents(path_translate, 0700) == 0)
            printf("Create folder: %d\n", page);
    }
    g_free(path_translate);

    if (stat(file_translate, &st) == -1) {
        GError *error = NULL;

        if (!g_file_set_contents(file_translate, text, -1, &error)) {
            //printf("Gagal tulis file: %s\n", file_translate);
            g_error_free(error);
        }
        g_free(file_translate);
        return text;
    }
    else {
        gchar *isi = NULL;
        ...
        if (ev_view_get_text_paragraphs_for_page (
                ...
                */
                for (i = 0; i < n_paragraphs; i++) {
                        ...
                        draw_overlay_paragraf(view, cr, paragraph_text,
                                 view_rect.x,
                                 view_rect.y,
                                 view_rect.width,
